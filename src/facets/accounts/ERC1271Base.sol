...
abstract contract ERC1271Base is EIP712Base {
    ...
    error ERC1271__InvalidSignature();
        ...
        returns (bool result)
    {
        ...
        if (t != uint256(0)) {
            // Try to get EIP712 domain, but handle case where it's not implemented
            try IERC5267(address(this)).eip712Domain() returns (
                bytes1,
                string memory name,
                ...
                // If EIP712 domain is not available, fall back to regular validation
                return false;
            }
        }
        /// @solidity memory-safe-assembly
    ...
    function _erc1271IsValidSignatureViaRPC(
        ...
        returns (bool result)
    {
        // Simple test-friendly implementation
        // In production, you might want the full gas-burning logic from Solady
