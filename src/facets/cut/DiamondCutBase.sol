...
library DiamondCutBase {
    ...
    function _replaceFacet(address facet, bytes4[] memory selectors) internal {
        ...
        for (uint256 i; i < selectorCount; ++i) {
            ...
            if (oldFacet == facet) {
                revert IDiamondCutBase.DiamondCut_FunctionFromSameFacetAlreadyExists(selector);
            }
            ...
            EnumerableSet.Bytes32Set storage oldFacetSelectors = ds.selectorsByFacet[oldFacet];
    ...
    function _validateFacetCut(IDiamond.FacetCut memory facetCut) internal view {
        ...
        if (facetCut.facetAddress != address(this) && facetCut.facetAddress.code.length == 0) {
            revert IDiamondCutBase.DiamondCut_InvalidFacet(facetCut.facetAddress);
        }

        if (facetCut.functionSelectors.length == 0) {
            revert IDiamondCutBase.DiamondCut_InvalidFacetSelectors(facetCut.facetAddress);
        }
    }
