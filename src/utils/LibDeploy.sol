...
library LibDeploy {
    ...
    /// @dev Multicall3 bytecode, used in test
    bytes internal constant MULTICALL3_BYTECODE =
    ...
    /// @notice Get the artifact path for a contract name with validation
    /// @param outDir Path to the artifacts directory (usually "out")
    /// @param name Name of the contract
    /// @return path The path to the contract artifact
        ...
        returns (string memory path)
    {
        ...
        revert(
            string.concat(
                "LibDeploy: Could not find artifact for '",
                name,
    ...
    {
        string memory artifactPath = getArtifactPath(outDir, name);
        return deployCode(artifactPath, args, salt);
    }
    ...
    {
        string memory artifactPath = getArtifactPath(outDir, name);
        return predictDeployCode(artifactPath, args, salt);
    }
