...
contract TestPlus is Brutalizer {
  ...
  /// @dev Canonical address of 0age's immutable create 2 factory.
  address internal constant _IMMUTABLE_CREATE2_FACTORY =
    0x0000000000FFe8B47B3e2130213B802212439497;

  /// @dev The bytecode of 0age's immutable create 2 factory.
  bytes internal constant _IMMUTABLE_CREATE2_FACTORY_BYTECODE =
  ...
  function __markAsGenerated(
    bytes32 typeId,
    bytes32 groupId,
    uint256 value
  ) private returns (bool isSet) {
  ...
  /// @dev Returns a random private key that can be used for ECDSA signing.
  function _randomUniquePrivateKey(
    uint256 groupId
  ...
  function __toBrutalizedAddress(
    address a
  ...
  /// @dev Returns a pseudorandom signer and its private key.
  /// The signer may have dirty upper 96 bits.
  function _randomUniqueSigner(
    uint256 groupId
  ) internal returns (address signer, uint256 privateKey) {
  ...
  /// The result may have dirty upper 96 bits.
  /// This function will not return an existing contract.
  function _randomUniqueNonZeroAddress(
    uint256 groupId
  ) internal returns (address result) {
  ...
  function _randomVmVar() internal returns (uint256 result) {
    ...
    if (t == 7) {
      ...
      assembly {
        ...
        result := mload(
          add(m, mod(and(y, 0xffff), add(returndatasize(), 0x20)))
        )
      }
      return result;
    }
  ...
  /// @dev Returns a pseudorandom address.
  function _randomUniqueHashedAddress(
    uint256 groupId
  ) internal returns (address result) {
  ...
  function __randomBytes(
    ...
    assembly {
      ...
      for {} 1 {} {
        ...
        mstore(
          add(result, 0x20),
          xor(calldataload(0x00), _TESTPLUS_RANDOMNESS_SLOT)
        )
  ...
  /// @dev Returns a random bytes string from 0 to 131071 bytes long.
  /// This function may return a previously returned result.
  function _randomBytesZeroRightPadded()
    internal
  ...
  /// @dev Deploys a contract via Nick's factory for testing.
  ...
  function _safeCreate2(
    ...
    if (!__hasCode(f)) __etch(f, _IMMUTABLE_CREATE2_FACTORY_BYTECODE);
    /// @solidity memory-safe-assembly
    assembly {
      let m := mload(0x40)
      let n := mload(initializationCode)
      ...
      if iszero(
        call(gas(), f, payableAmount, add(m, 0x1c), add(n, 0x64), m, 0x20)
      ) {
        returndatacopy(m, 0x00, returndatasize())
